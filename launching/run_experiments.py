...
def run_experiment(arg):
    ...
    task_dir = os.path.join(BASE_TASK_DIR, map_name)
    config_name = f"{alg_name}{BASE_CONFIG_FILE}.xml"
    h, w, cs, grid_map, obstacles = xml_io.read_xml_map(map_path)
    ...
    for task_id in range(task_num):
        ...
        for agents_num in range(agents_from, agents_to + 1, agents_step):
            ...
            vis_graph_obj = ctp_swap.VisibilityGraph(obstacles, goals[:agents_num, 0:2], 0.7)
            ...
            print(
                f"Task{task_id+1:>4} / {task_num:<4}-> ",
                str(summary),
                f"    (Algorithm:{alg_name:^15}, Map:{map_name})",
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        nargs="*",
        help="The algorithms that will participate in the experiments.",
    )
